...
from scripts.parser.jobviews import JobViewParser


class Command(BaseCommand):

    help = "Downloads and parses jobviews"
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest='start_date',
            default='',
            help='Apache log file start date'
        )
        parser.add_argument(
            '--end_date',
            action='store',
    ...
    def handle(self, *args, **kwargs):
        ...
        for f in files:
            if f.endswith('.gz'):
                ...
                f = f[:-3]
            parser = JobViewParser(filepath=f)
            parser.parse()
